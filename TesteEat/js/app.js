(() => {
    ...
    (() => {
        ...
        timepickerContainer.classList.add("qs-timepicker-container");
        const timeInputParent = timeInput.closest(".form-reservation__time");
        timeInputParent.appendChild(timepickerContainer);
        const timeGrid = document.createElement("div");
        timeGrid.classList.add("qs-timepicker-grid");
        timepickerContainer.appendChild(timeGrid);
        for (let hour = 7; hour <= 18; hour++) {
            const minutesArray = hour === 18 ? [ 0 ] : [ 0, 30 ];
            for (let minute of minutesArray) {
                const timeButton = document.createElement("button");
                const timeText = `${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`;
                timeButton.textContent = timeText;
                timeButton.value = timeText;
        ...
        document.addEventListener("click", (function(event) {
            if (!timeInput.contains(event.target) && !timepickerContainer.contains(event.target)) timepickerContainer.style.display = "none";
        }));
        window["FLS"] = true;
        menuInit();
        showMore();
